...
void Application::import_reviews(string table_name, string key_name) {
	using json = nlohmann::json;
	ifstream table_file(table_name + ".json");
	json j_table;
	table_file >> j_table;
	...
	ofstream output_file(table_name + "_review.json");

	Table& review_table = this->get_table("review");
	...
	for (int k = 0; k < max_search; k++) {
		...
		for (int i = 0; i < review_table.get_size(); i++) {
			string business_id = review_table.return_record(i).get_record_entry(attri_pos);
			if (j_table[key_name] == business_id) {
				output_file << j_table << endl;
				break;
			}
		}
		table_file >> j_table;
